...
theme_bw() +
  ...
  annotate("text", x = Inf, y = Inf, label = expression(bold("R = 0.923")),
...
Calibration_plot_Y <- ggplot(Calibration_pb_yield, aes(x = species, y = 1 - ratio, fill = species)) +
  ...
  annotate("text", x = Inf, y = min(Calibration_pb_yield$ratio),
           label = expression(bold("pb = -0.420")), parse = TRUE,
...
Calibration_plot_B <- ggplot(Calibration_pb_biomass, aes(x = species, y = 1 - ratio, fill = species)) +
  ...
  geom_hline(yintercept = 0, color = "black", size = 0.1) +
  annotate("text", x = length(unique(Calibration_pb_biomass$species)) + 0.5, y = max(1 - Calibration_pb_biomass$ratio) ,
...
```{r}
# Run the model to a dynamic equilibrium with seal predation included
# - effort = initial_effort(params_with_seal): hold effort constant during spin-up
# - t_max = 500: long spin-up period to allow system to stabilize under new predation structure
params_spin2<-projectToSteady(params_with_seal,effort=initial_effort(params_with_seal),t_max=500)
# Project the model forward using a full time series of effort (1970–2010, etc.)
# - effort_with_seal: should be a time-varying effort matrix/data.frame
sim2<- project(params_spin2, effort = effort_with_seal)
# Plot total biomass and yield time series (summary plot)
plot(sim2)

# Plot species biomass over time
plotBiomass(sim2)
# Plot size spectra at the end of simulation
plotSpectra(sim2)
# Plot species diet composition by predator and size class
# Helps verify seal predation and diet shifts
plotDiet(params_with_seal)
# Plot growth curves under new conditions (with seal predation)
# - Species are plotted in separate panels
plotGrowthCurves(params_with_seal, species_panel = TRUE, max_age = 50)
