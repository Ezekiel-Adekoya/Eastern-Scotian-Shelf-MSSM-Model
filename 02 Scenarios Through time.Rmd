...
biomass_plot_with <- ggplot(time_series, aes(x = Year, y = Biomass, colour = Scenario, fill = Scenario)) +
  ...
  scale_colour_manual(name = NULL,  # Removes legend title
                      ...
                      labels = c("Calibration", "Fishing without additional natural mortality", "Fishing with additional natural mortality")) +
...
yield_plot_with = ggplot(time_series_c, aes(x=Year, y=Yield, colour=scen, fill=scen)) +   
  facet_wrap(~factor(Species, levels = c("HERRING", "WITCH FLOUNDER", "REDFISH UNSEPARATED", 
                                         "YELLOWTAIL FLOUNDER", "SILVER HAKE", "AMERICAN PLAICE", 
                                         "GREENLAND HALIBUT", "HADDOCK", "SPINY DOGFISH", 
                                         "WHITE HAKE", "ATLANTIC COD", "ATLANTIC HALIBUT")), 
              scales="free") +   
  ...
  scale_colour_manual(name = NULL,  
                      ...
                      labels = c("Calibration", 
                                 "Fishing without additional natural mortality", 
                                 "Fishing with additional natural mortality")) +   
...
# combine
sf1$scenario <- "Fishing without additional natural mortality"
sf2$scenario <- "Fishing with additional natural mortality"
...
combined_data$Legend <- factor(combined_data$Legend, 
                               levels = c("Resource","SANDLANCES", "HERRING", "REDFISH UNSEPARATED", 
...
trophic_cascades_plot <- ggplot(combined_data, aes(x = w, y = rel_diff, colour = scenario)) +
  ...
  facet_wrap(~Legend, scales = "free") +  # Use 'Legend' for facet_wrap if that’s the intended grouping
