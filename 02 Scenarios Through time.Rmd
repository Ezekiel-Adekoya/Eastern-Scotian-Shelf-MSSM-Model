...
time_seriess <- time_series %>%
  rename(Scenario = scen) %>%
  mutate(Species = fct_recode(Species, "SAND LANCE" = "SANDLANCES"))
time_seriess <- time_seriess %>%
  mutate(
    Biomass =   (Biomass/10^6),
    sim_mean =  (sim_mean/10^6),
    sim_lower = (sim_lower/10^6),
    sim_upper = (sim_upper/10^6)
  )
biomass_plot <- ggplot(time_seriess, aes(x = Year, y = Biomass)) +
  facet_wrap(~factor(Species, levels = c(
    "AMERICAN PLAICE", "ATLANTIC COD", "ATLANTIC HALIBUT", "ATLANTIC HERRING",
    "REDFISH UNSEPARATED", "GREENLAND HALIBUT", "HADDOCK", "SAND LANCE",
    "SILVER HAKE", "SPINY DOGFISH", "WHITE HAKE", "WITCH FLOUNDER", "YELLOWTAIL FLOUNDER"
  )), scales = "free") +
  ...
  # Observed points
  geom_point(aes(shape = "Observed Biomass"), colour = "black", size = 1.2) +
  
  # Ribbon for simulations
  geom_ribbon(aes(ymin = sim_lower, ymax = sim_upper, fill = Scenario), alpha = 0.3, colour = NA) +
  
  # Line for simulation mean
  geom_line(aes(y = sim_mean, colour = Scenario), size = 1) +
  
  # Scales for colours/fills/shapes
  scale_colour_manual(
    name = NULL,
  ...
  scale_shape_manual(  name = NULL,values = c("Observed Biomass" = 16), guide = guide_legend(order = 1)) +
...
ggsave(filename = paste0(save_dir, "Ess_Output/biomass_plot.jpeg"), plot = biomass_plot,width = 16, height = 11, units = "in",dpi = 400)
biomass_plot
```
...
species<-levels(time_series_c $Species)<- c("AMERICAN PLAICE", "ATLANTIC COD","ATLANTIC HALIBUT", "ATLANTIC HERRING","REDFISH UNSEPARATED", "GREENLAND HALIBUT", "HADDOCK","SAND LANCE", "SILVER HAKE", "SPINY DOGFISH",  "WHITE HAKE", "WITCH FLOUNDER", "YELLOWTAIL FLOUNDER")
time_series_c <- time_series_c%>%
  mutate(
    Yield =   (Yield/10^6),
    sim_mean =  (sim_mean/10^6),
...
yield_plot <- ggplot(time_series_c, aes(x=Year, y=Yield, colour=scen, fill=scen)) +   
  facet_wrap(~factor(Species, levels = c("AMERICAN PLAICE", "ATLANTIC COD", "ATLANTIC HALIBUT", "ATLANTIC HERRING",
                                         "REDFISH UNSEPARATED", "GREENLAND HALIBUT", "HADDOCK", "SAND LANCE",
                                         "SILVER HAKE", "SPINY DOGFISH", "WHITE HAKE", "WITCH FLOUNDER", "YELLOWTAIL FLOUNDER")), 
             scales="free") +   
  geom_point(aes(shape = "Observed catch"), colour="black", size=1.2) +   
  geom_ribbon(aes(ymin=sim_lower, ymax=sim_upper), alpha=0.3, colour=NA) +   
  geom_line(aes(y=sim_mean)) +   
  scale_colour_manual(values = c("grey70", "#4575B4", "#D73027"),
                      labels = c("Reference period", 
                                 "Fishing without additional predation mortality", 
                                 "Fishing with additional predation mortality")) +   
  scale_fill_manual(values = c("grey70", "#4575B4", "#D73027"),
                    labels = c("Reference period", 
                               "Fishing without additional predation mortality", 
                               "Fishing with additional predation mortality")) +   
  scale_shape_manual(name = NULL, values = c("Observed catch" = 16)) +
  labs(y = "Catches (MT)", x = "Year", fill = NULL, colour = NULL) +   
  geom_vline(xintercept = 1986, linetype="dashed", color = "black") +   
  theme_pubr() +   
  theme(
    legend.position = "bottom",
...
ggsave(filename = paste0(save_dir, "yield_plot.jpeg"), plot = yield_plot,
       width = 17, height = 10, units = "in",dpi = 400)
```
...
combined_data <- combined_data %>%
  mutate(Legend = case_when(
    ...
    Legend == "HERRING(ATLANTIC)" ~ "ATLANTIC HERRING",
...
combined_data$Legend <- factor(combined_data$Legend, 
  levels = c("Resource", "SAND LANCE", "ATLANTIC HERRING", "HADDOCK", "YELLOWTAIL FLOUNDER", 
