...
library(dplyr)
library(forcats)
library(scales)      # Needed for trans_breaks and log formatting
library(ggtext)      # Optional for styled axis labels
# 1. Compute summary statistics for each scenario
# ---------------------------------------------
# Reference period (1970–1985)
spectra_reference <- plotSpectra(sims_scen1[[1]], return_data = TRUE, time_range = 1970:1985)

# Scenario 1: no additional predation mortality (1995–2010)
spectra_scen1 <- plotSpectra(sims_scen1[[1]], return_data = TRUE, time_range = 1986:2010)

# Scenario 2: with additional predation mortality (1995–2010)
spectra_scen2 <- plotSpectra(sims_scen2[[1]], return_data = TRUE, time_range = 1986:2010)


# Scenario 1 relative difference
sf1_nonres <- left_join(spectra_reference, spectra_scen1, by = c("w", "Species")) %>%
  mutate(rel_diff = (value.y - value.x)/((value.x + value.y)/2),
         scenario = "Fishing without additional mortality")
# Scenario 2 relative difference
sf2_nonres <- left_join(spectra_reference, spectra_scen2, by = c("w", "Species")) %>%
  mutate(rel_diff = (value.y - value.x)/((value.x + value.y)/2),
         scenario = "Fishing with additional mortality")

# Scenario 1
# Community (non-resource) average for Scenario 1
sim_mean_c  <- sf1_nonres  |> group_by(w) |> summarise(mean_value = sum(rel_diff)) |> ungroup()
sim_lower_c <- sf1_nonres  |> group_by(w) |> summarise(mean_value = min(rel_diff)) |> ungroup()
sim_upper_c <- sf1_nonres  |> group_by(w) |> summarise(mean_value = max(rel_diff)) |> ungroup()

# Scenario 2# Community (non-resource) average for Scenario 2
sim_mean2_c  <- sf2_nonres |> group_by(w) |> summarise(mean_value =sum(rel_diff)) |> ungroup()
sim_lower2_c <- sf2_nonres |> group_by(w) |> summarise(mean_value = min(rel_diff)) |> ungroup()
sim_upper2_c <- sf2_nonres |> group_by(w) |> summarise(mean_value = max(rel_diff)) |> ungroup()


# 2. Create summary dataframes for each scenario
# ---------------------------------------------
sim1_c <- data.frame(
  w         = sim_mean_c$w,
  sim_mean  = sim_mean_c$mean_value,
  sim_lower = sim_lower_c$mean_value,
  sim_upper = sim_upper_c$mean_value,
  scen      = "Scen1"
)

sim2_c <- data.frame(
  w         = sim_mean2_c$w,
  sim_mean  = sim_mean2_c$mean_value,
  sim_lower = sim_lower2_c$mean_value,
  sim_upper = sim_upper2_c$mean_value,
  scen      = "Scen2"
)

time_series_c <- rbind(sim1_c, sim2_c)
# ============================================================================
# Optional: Clean or recode scenario/species labels
# ============================================================================
# (Skip this if you already did your recoding elsewhere)
time_series_c <- time_series_c %>%
  mutate(
    scen = recode(scen,
                  "Scen1" = "Fishing without additional mortality",
                  "Scen2" = "Fishing with additional mortality")
  )
# ============================================================================
# Create the Plot
# ============================================================================
spectra_plot2 <- ggplot(time_series_c, aes(x = w, y = sim_mean, colour = scen, fill = scen)) +
  
  # Confidence ribbon (uncertainty)
  geom_ribbon(aes(ymin = sim_lower, ymax = sim_upper),
  ...
  # Optional: Y-axis tidy scientific notation
  scale_y_continuous(labels = label_number(scale_cut = cut_short_scale())) +
  
  # Colors (customizable palette)
  scale_colour_manual(values = c(
 "Fishing without additional mortality" = "#4575B4",
  "Fishing with additional mortality" = "#D73027"
  )) +
  scale_fill_manual(values = c(
 "Fishing without additional mortality" = "#4575B4",
  "Fishing with additional mortality" = "#D73027"
  )) +
  labs( x = "Size (g)",y = "Symmetric relative biomass difference") +
  theme_minimal(base_size = 12) +
   theme_bw() +
  geom_hline(yintercept = 0, linetype = 1, colour = "darkgrey", linewidth = 0.75) +
  theme(
    axis.title = element_text(face = "bold", size = 10),
    axis.text.x = element_text(face = "bold", size = 10),  # <-- x-axis numbers bold
  ...
  annotate(
    "text", x = min(time_series_c$w), y = max(time_series_c$sim_upper, na.rm = TRUE),
    label = "(b)", hjust = 1, vjust = -0.8, fontface = "bold", size = 6
  )
...
# ============================================================================
ggsave("spectra_plot2_publication.pdf", spectra_plot2, width = 7, height = 5, units = "in")

```


Community spectra by Scenario
```{r}
# ---------------------------------------------
# 1. Compute summary statistics for each scenario
# ---------------------------------------------
...
# -----------------------------
# 2. Separate Resource and non-Resource
# -----------------------------

# Scenario 1
# Community (non-resource) average for Scenario 2
sf_ref <- spectra_reference %>% filter(Species == "Resource")
sim_mean_r  <- sf_ref  |> group_by(w) |> summarise(mean_value = sum(value)) |> ungroup()
sim_lower_r <- sf_ref  |> group_by(w) |> summarise(mean_value = min(value)) |> ungroup()
sim_upper_r <- sf_ref  |> group_by(w) |> summarise(mean_value = max(value)) |> ungroup()
# Scenario 1
# Community (non-resource) average for Scenario 1
sf_nonres <- spectra_scen1 %>% filter(Species != "Resource")
sim_mean_c  <- sf_nonres  |> group_by(w) |> summarise(mean_value = sum(value)) |> ungroup()
sim_lower_c <- sf_nonres  |> group_by(w) |> summarise(mean_value = min(value)) |> ungroup()
sim_upper_c <- sf_nonres  |> group_by(w) |> summarise(mean_value = max(value)) |> ungroup()

# Scenario 2# Community (non-resource) average for Scenario 2
sf2_nonres <- spectra_scen2 %>%filter(Species != "Resource")
sim_mean2_c  <- sf2_nonres |> group_by(w) |> summarise(mean_value = sum(value)) |> ungroup()
sim_lower2_c <- sf2_nonres |> group_by(w) |> summarise(mean_value = min(value)) |> ungroup()
sim_upper2_c <- sf2_nonres |> group_by(w) |> summarise(mean_value = max(value)) |> ungroup()
...
# ---------------------------------------------
# 3. Combine both scenarios into a single dataframe
# ---------------------------------------------
time_series_c <- rbind(sim1_c, sim2_c)

# Map scenario codes to descriptive labels
time_series_c$scen <- recode(time_series_c$scen,
                             "Scen1" = "Fishing without additional mortality",
                             "Scen2" = "Fishing with additional mortality")
...
# ---------------------------------------------
# 4. Define line colors
# ---------------------------------------------
linecolours <- c(
  "Fishing without additional mortality" = "#4575B4",
  "Fishing with additional mortality" = "#D73027"
)
#common_limits <- range(combined_sf$w, time_series_c$w)
# ---------------------------------------------
# 5. Create the ggplot
# ensure consistent y-axis or x-axis limits if you want identical panel size
y_limits <- range(time_series_c$sim_mean, na.rm = TRUE)

spectra_plot <- ggplot(time_series_c, aes(x = w, y = sim_mean, colour = scen, fill = scen)) +
  geom_ribbon(aes(ymin = sim_lower, ymax = sim_upper),
              alpha = 0.1, colour = NA, show.legend = FALSE) +
  geom_line(linewidth = 1.2, alpha = 0.8) +
  scale_x_log10(
  ...
  annotate(
    "text", x = min(time_series_c$w), y = max(time_series_c$sim_upper, na.rm = TRUE),
    label = "(a)", hjust = 1, vjust = -6.5, fontface = "bold", size = 6
  )
...
# Display the Plot
# ============================================================================
Spectra_community = spectra_plot/spectra_plot2
ggsave(filename = paste0(save_dir, "Spectra_community.jpeg"), plot = Spectra_community, width = 11, height = 7.5, units = "in", dpi = 400)
