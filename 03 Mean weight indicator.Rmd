...
species<-levels(time_mw$Species)<- c("AMERICAN PLAICE", "ATLANTIC COD","ATLANTIC HALIBUT", "ATLANTIC HERRING","REDFISH UNSEPARATED", "GREENLAND HALIBUT", "HADDOCK","SAND LANCE", "SILVER HAKE", "SPINY DOGFISH",  "WHITE HAKE", "WITCH FLOUNDER", "YELLOWTAIL FLOUNDER")
time_mw <- time_mw %>%
...
Mean_weight_plot <- ggplot(time_mw, aes(x = Year, y = Mean_weight, colour = Scenario, fill = Scenario)) +
  facet_wrap(~factor(Species, levels = c("AMERICAN PLAICE", "ATLANTIC COD","ATLANTIC HALIBUT", "ATLANTIC HERRING","REDFISH UNSEPARATED", "GREENLAND HALIBUT", "HADDOCK","SAND LANCE", "SILVER HAKE", "SPINY DOGFISH",  "WHITE HAKE", "WITCH FLOUNDER", "YELLOWTAIL FLOUNDER")), scales="free") +
 # geom_point(aes(y = Mean_weight), colour = "black", size = 0.5) +
   geom_point(aes(shape = "Mean_weight"), colour = "black", size = 1.2) +
  ...
  scale_shape_manual(name =  NULL, values = c("Mean_weight" = 16),labels = c("Observed mean weight"))+
...
ggsave(filename = paste0(save_dir, "Mean_weight_plot.jpeg"), plot = Mean_weight_plot,width = 16, height = 11, units = "in",dpi = 400)
Mean_weight_plot
```
...
library(ggplot2)
library(ggpubr)
cor_plot <- ggplot(cor_results_long, aes(x = value, y = Variable, fill = scen)) +
  geom_violin(trim = FALSE, scale = "width", alpha = 0.8, color = "white", linewidth = 0.8) +
  ...
  scale_fill_manual(values = c(
    "Fishing without additional predation mortality" = "#2E86AB",
    "Fishing with additional predation mortality" = "#E63946"
  )) +
  labs(
  ...
  theme(
    ...
    axis.title.x = element_text(size = 12, face = "bold", color = "grey20", margin = margin(t = 10)),
    axis.title.y = element_text(size = 12, face = "bold", color = "grey20", margin = margin(r = 10)),
    legend.text = element_text(size = 10, face = "bold", color = "grey20"),
    legend.position = "bottom",
...
cor_plot
 ggsave(filename = paste0(save_dir, "cor_plot_option.jpeg"), plot = cor_plot,width = 11, height = 7.5, units = "in",dpi = 400)
```
```
